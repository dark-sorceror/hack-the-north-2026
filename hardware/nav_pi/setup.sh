...
#
#   bash hardware/nav_pi/setup.sh                  # bridge with the FAKE wheels, on every boot
#   bash hardware/nav_pi/setup.sh --driver real    # the real DDSM115 wheels (any run_bridge.py flags work)
#
...
sudo install -m 0644 "$HERE/hardware/nav_pi/99-retriever-d435i.rules" /etc/udev/rules.d/99-retriever-d435i.rules
...
# A bridge started by tools/deploy/nav_pi.sh --run holds :7777, and the service
...
if [ -f "$PIDFILE" ]; then
  ...
  if [ -n "$pid" ] && [[ $cmd == *run_bridge.py* || $cmd == *fake_pi.py* ]]; then
...
sed -e "s|__USER__|$USER|" -e "s|__HOME__|$HOME|" -e "s|__BRIDGE_ARGS__|$BRIDGE_ARGS|" \
    ...
    "$HERE/hardware/nav_pi/retriever-bridge.service" \
...
echo "  restart:  sudo systemctl restart retriever-bridge   (after every tools/deploy/nav_pi.sh)"
echo "  from the laptop:"
echo "    tools/diagnostics/pi_doctor.sh --host $USER@$(hostname).local"
echo "    tools/deploy/nav_pi.sh --host $USER@$(hostname).local --skip-tests --link-test 120"
