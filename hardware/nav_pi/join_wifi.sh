...
#
#   sudo bash hardware/nav_pi/join_wifi.sh "Hao's iPhone" PASSWORD          # priority 10
#   sudo bash hardware/nav_pi/join_wifi.sh "TeamRouter" PASSWORD 20          # preferred when in range
#   sudo bash hardware/nav_pi/join_wifi.sh "ASUS_30" "" 20                   # an OPEN network: "" password
#
...
if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
  echo "usage: sudo bash hardware/nav_pi/join_wifi.sh \"NETWORK NAME\" PASSWORD [PRIORITY]" >&2
  exit 2
fi
if [ "$(id -u)" -ne 0 ]; then
