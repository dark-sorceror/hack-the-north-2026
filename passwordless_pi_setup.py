...
Usage:
    python passwordless_pi_setup.py [OPTIONS]
...
class PasswordlessPiSetup:
    ...
    def step2_remove_passphrase(self, current_passphrase):
        ...
        if not current_passphrase:
            ...
            print("\nCommand:")
            print(f'  python passwordless_pi_setup.py --passphrase "YOUR_PASSPHRASE"')
            return False
...
Examples:
  ...
  python passwordless_pi_setup.py --passphrase "my-passphrase"
  
  # Also enable auto-login at boot
  python passwordless_pi_setup.py --passphrase "my-passphrase" --auto-login
  
  # Test connection without making changes
  python passwordless_pi_setup.py --test-only
  
  # Custom Pi user/host
  python passwordless_pi_setup.py --pi-user pi --pi-host 192.168.1.102 --passphrase "pass"
        """
    )
