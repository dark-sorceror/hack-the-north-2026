...
"""DDSM115 wheel check: find the RS485 bus, list the motors, spin them one at a time.
    ...
    python3 scripts/wheel_check.py scan                    # read-only: IDs, mode, temp, errors
    python3 scripts/wheel_check.py spin 1 --yes            # motor 1, 20 rpm robot-forward, 2 s
    python3 scripts/wheel_check.py spin 3 -30 --seconds 3 --yes
    python3 scripts/wheel_check.py sides --yes             # left/right IDs? + is forward?
    python3 scripts/wheel_check.py rev 1 --yes             # one turn: checks counts_per_rev
...
def sides_verdict(answers: dict[str, tuple[str, str, int, int]], left: tuple[int, ...],
    ...
    if ok:
        say(f"\nCONFIRMED. Bridge flags for this chassis (scripts/fake_pi.py): {flags}")
        return 0
