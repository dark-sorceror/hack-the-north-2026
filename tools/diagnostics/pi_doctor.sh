...
#
#   tools/diagnostics/pi_doctor.sh                                   # hao@hao.local, bridge :7777
#   tools/diagnostics/pi_doctor.sh --host fe80::8aa2:9eff:fea9:d084%en9
#   tools/diagnostics/pi_doctor.sh --host pi@retriever-pi.local --port 7777 --iface en9
#
...
#              3  Pi reachable over SSH, bridge not answering (tools/deploy/nav_pi.sh --run)
#              1  Pi not usable -- the VERDICT line says why
#              2  usage error
#
# Env defaults: PI_HOST PI_USER PI_PORT PI_IFACE PI_DOCTOR_BUDGET.
# tools/deploy/nav_pi.sh sources this file for bounded, tcp_probe and PI_SSH_OPTS;
...
doctor_usage() {
  cat <<'EOF'
usage: tools/diagnostics/pi_doctor.sh [--host HOST] [--user USER] [--port PORT] [--iface IFACE] [--budget SECS]
...
doctor_main() {
          ...
          if [ $darwin = 1 ] && [ "$carrier" = yes ] && [ "$c" -ge 1 ]; then
            ...
            if [ -z "$others" ]; then
              ...
              _say fail "$why; something else answers on $iface ($others): the Pi's address changed?" \
                "try: tools/diagnostics/pi_doctor.sh --host $(echo "$others" | awk '{ print $1 }')"
            fi
        ...
        *'another client'*)
        ...
        REFUSED)
          ...
          if [ -n "$pi_listen" ]; then
            ...
            bfix="restart it bound to all addresses: tools/deploy/nav_pi.sh --run"
  ...
  if [ "$bstate" = down ]; then
    ...
    bfix="tools/deploy/nav_pi.sh --run"
