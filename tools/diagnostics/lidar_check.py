...
the way the safety bubble thinks it is?
    ...
    python3 tools/diagnostics/lidar_check.py                                # auto port; MOTOCTL tied high
    python3 tools/diagnostics/lidar_check.py --port /dev/ttyAMA0 --motor-pin 18   # the robot's wiring
    python3 tools/diagnostics/lidar_check.py --motor-pin 18 --find-front         # measure the mount yaw
    python3 tools/diagnostics/lidar_check.py --motor-pin 18 --record-mask 10     # propose a self-mask
    python3 tools/diagnostics/lidar_check.py --fake                              # no hardware at all
...
many rays came back. Needs pyserial (python3-serial; hardware/nav_pi/setup.sh installs it)
