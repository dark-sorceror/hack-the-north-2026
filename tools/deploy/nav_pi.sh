...
#
#   tools/deploy/nav_pi.sh                          # preflight, sync, Pi-side tests
#   tools/deploy/nav_pi.sh --run                    # ... then (re)start the bridge on the Pi
#   tools/deploy/nav_pi.sh --run --link-test 30     # ... then hold a zero-velocity link for 30 s
#   tools/deploy/nav_pi.sh --stop                   # stop the bridge started by --run
#
# Steps, each bounded in time and failing fast with the reason:
#   1. preflight   tools/diagnostics/pi_doctor.sh: can we reach the Pi at all, and if not, why
#   2. sync        rsync src/ scripts/ hardware/nav_pi/ tests/ -> ~/retriever on the Pi
...
#   5. --link-test tools/diagnostics/link_test.py from the laptop for N seconds, prints its verdict
#
...
REPO="$(cd "$HERE/../.." && pwd)"
# shellcheck source=tools/diagnostics/pi_doctor.sh
. "$REPO/tools/diagnostics/pi_doctor.sh"
...
SYNC_DIRS="src scripts hardware tools tests"

usage() {
  cat <<'EOF'
usage: tools/deploy/nav_pi.sh [--host HOST] [--user USER] [--port PORT]
...
Deploy the bridge to the Raspberry Pi and check it, in one command:
  preflight (pi_doctor.sh) -> rsync src/ scripts/ hardware/nav_pi/ tests/ to ~/retriever
  ...
  --link-test SECS   hold a zero-velocity link from this laptop for SECS seconds
                     and print tools/diagnostics/link_test.py's verdict
  ...
  -- ARGS...         passed to scripts/run_bridge.py, e.g. -- --estop-pin 17

  e.g.  tools/deploy/nav_pi.sh --run --link-test 30
        tools/deploy/nav_pi.sh --stop
...
_remote_stop() {
  ...
  if [ -f run/bridge.pid ]; then
    ...
    # Only ever kill what we started: the pid must still be a current or pre-rename bridge.
    if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null &&
      tr '\0' ' ' < "/proc/$pid/cmdline" 2>/dev/null | grep -Eq '(run_bridge|fake_pi)\.py'; then
      kill -TERM "$pid"                     # the bridge stops the motors on SIGTERM
...
_remote_start() {
  ...
  setsid nohup sh -c 'echo $$ > run/bridge.pid; exec "$@"' sh \
    python3 -u scripts/run_bridge.py "$hostarg" --port "$port" "$@" \
...
main() {
  ...
  # 1. preflight --------------------------------------------------------------
  if [ "$preflight" = 1 ]; then
    say "preflight: tools/diagnostics/pi_doctor.sh --host $PI_USER@$PI_HOST"
    "$REPO/tools/diagnostics/pi_doctor.sh" --host "$PI_USER@$PI_HOST" --port "$PI_PORT"
    rc=$?
  ...
  if [ "$sync" = 0 ]; then
    ...
    case $rc in
      ...
      *) die 3 "sync: rsync exit $rc (see above). Check: tools/diagnostics/pi_doctor.sh --host $PI_USER@$PI_HOST" ;;
    esac
  fi
  ...
  # 4. --run --------------------------------------------------------------------
  if [ "$run" = 1 ]; then
    say "bridge: (re)starting scripts/run_bridge.py on $PI_HOST:$PI_PORT (bind '${bind:-*}' = ${bind:-all addresses, IPv4 + IPv6})"
    ...
    case $? in
      ...
      *) die 5 "the bridge runs on the Pi but does not answer from this laptop at $(hostport): $BRIDGE_PROBE. Diagnose: tools/diagnostics/pi_doctor.sh --host $PI_USER@$PI_HOST" ;;
    esac
    echo "    log:   ssh $PI_USER@$PI_HOST tail -f $REMOTE_DIR/run/bridge.log"
    echo "    stop:  tools/deploy/nav_pi.sh --stop$hostflag"
  fi
  ...
  if [ "$link" != 0 ]; then
    ...
    if [ "$run" != 1 ]; then
      ...
      case $? in
        ...
        *) die 6 "the bridge is not answering at $(hostport) ($BRIDGE_PROBE). Start it: tools/deploy/nav_pi.sh --run --link-test $link" ;;
      esac
    fi
    local out="${TMPDIR:-/tmp}/pi_deploy_link.$$"
    bounded $((link + 30)) "$py" -u "$REPO/tools/diagnostics/link_test.py" "$(hostport)" --seconds "$link" 2>&1 | tee "$out"
    rc=${PIPESTATUS[0]}
    ...
    case $rc in
      0)
        if [ "${closed:-0}" -gt 0 ]; then
          ...
          die 6 "link test: the link $silent_msg, far past the 300 ms watchdog -- the verdict above misses a stall at the very end. Diagnose: tools/diagnostics/pi_doctor.sh$hostflag"
        fi
        ;;
      ...
      *)
        if [ "${closed:-0}" -gt 0 ]; then
          ...
          die 6 "link test: the link $silent_msg -- an outage, not jitter; no watchdog setting fixes it. If preflight warned that NetworkManager is waiting for DHCP, that is the cause: eth0 drops every 45 s. Diagnose: tools/diagnostics/pi_doctor.sh$hostflag"
        elif [ "$tripped" = 1 ]; then
          ...
          die 6 "link test could not run (exit $rc, see above). Diagnose: tools/diagnostics/pi_doctor.sh$hostflag"
        fi ;;
    esac
  fi
