...
"""The central Pi's mission loop: a spoken sentence in, wheels and an arm out.
    ...
    python3 scripts/run_mission.py --dry-run                    # no hardware at all
    python3 scripts/run_mission.py --nav 10.0.0.50:8791 --fake-detect 1.6,0.2
    python3 scripts/run_mission.py --nav 10.0.0.50:8791 --arm sunrise@10.0.0.112
...
class Arm:
    ...
    The board's own contract (hardware/so101/ARM_BOARD.md): pickup, then handoff, and
    ...
    def __init__(self, target: str | None, timeout_s: float = 60.0,
                 python: str = "~/hiwonder-SoArm-101/.venv/bin/python") -> None:
        self.target = target
        self.timeout_s = timeout_s
        self.python = python
    ...
    def handoff(self) -> tuple[bool, str]:
        ok, detail = self._ssh(
            f"{self.python} ~/arm_poses.py goto handoff --hold-gripper --duration 6", 30.0)
...
def run_once(heard: Heard, *, voice: Voice, detector: Detector, nav: Nav, arm: Arm,
    ...
    if seen is None:
        ...
        return False
    if not all(math.isfinite(v) for v in (seen.x, seen.y, seen.age_s)) or not (0 <= seen.age_s <= MAX_DETECTION_AGE_S):
    ...
    for attempt in range(MAX_NUDGES + 1):
        ...
        if not all(math.isfinite(v) for v in (again.x, again.y, again.age_s)) or not (0 <= again.age_s <= MAX_DETECTION_AGE_S):
            say("The new picture is too old or invalid to trust.")
            nav.halt()
            return False
        ...
        try:
            ...
            for auto in nav.watch(timeout_s=25.0):
                nudged = auto.get("status") == "arrived"
            if not nudged:
                say("I couldn't get close enough.")
                nav.halt()
                return False
        except NavError as exc:
            say(str(exc))
    ...
    try:
        ...
        home_arrived = False
        for auto in nav.watch():
            home_arrived = auto.get("status") == "arrived"
        if not home_arrived:
            nav.halt()
            say("I couldn't get home; I'm keeping hold of the object.")
            return False
    except NavError as exc:
        nav.halt()
        say(str(exc))
...
class DryDetector(FixedDetector):
    """Simulate a target moving within reach after the dry-run approach."""

    def locate(self, label: str) -> Seen:
        seen = super().locate(label)
        self.x, self.y = 0.20, 0.0
        return seen
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.fake_detect:
        ...
        from retriever.perception.floor import DepthStreamFrames, FloorDetector, parse_mount
        ...
        print("no detector wired: pass --fake-detect X,Y, or plug the real one in "
              "in src/retriever/mission.py", file=sys.stderr)
        return 2
