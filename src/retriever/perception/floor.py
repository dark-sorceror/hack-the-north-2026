...
"""Find the goose in a camera stream and say where it is, in metres, on the floor.
    ...
    python3 scripts/detect_floor.py --url http://hao.local:8790/stream \
        --mount 0.15,0.20,12 --target goose --show

This is the real `Detector` for `scripts/run_mission.py` -- the thing `--fake-detect`
...
class Seen:
    """Matches scripts/run_mission.py's Seen: robot frame, +x forward, +y left."""
    x: float
    y: float
    age_s: float
    label: str = ""
